...
class Login extends Component {
  ...
  doLogin(email, password) {
    ...
    axios.post(env.API_URL + '/auth/login', userData)
      .then((res) => {
        //use res from server
        ...
        else if (res.data.buycoin === true) {
          setTimeout(this.props.history.push("/Buy"), 1000);
        }
        else {
  ...
  doRegister(email, password) {
    var userData = { email, password };
    axios.post(env.API_URL + '/auth/register', userData)
      .then((res) => {
        //use res from server
        if (res.data.success === true) {
